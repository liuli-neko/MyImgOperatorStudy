...
/*pyramid表示高斯金字塔或者DOG金字塔
 ...
 str表示是高斯金字塔还是DOG金字塔
*/
void myDisplay::mosaic_pyramid(const vector<vector<Mat>> &pyramid,
                               Mat &pyramid_image, int nOctaceLayers,
                               string str) {
  //获得每组金字塔图像大小
  ...
  for (auto beg = pyramid.cbegin(); beg != pyramid.cend(); ++beg, ++i) {
    ...
    for (auto it = (*beg).cbegin(); it != (*beg).cend(); ++it) {
      ...
      Mat temp(pyramid_image,
               Rect(accumulate_col - temp_size[i][1],
                    accumulate_row - temp_size[i][0], it->cols, it->rows));
      Mat temp_it;
 ...
 */
void myDisplay::write_mosaic_pyramid(const vector<vector<Mat>> &gauss_pyr_1,
  ...
  mosaic_pyramid(gauss_pyr_1, gauss_image_1, nOctaveLayers,
                 string("高斯金字塔"));
  mosaic_pyramid(gauss_pyr_2, gauss_image_2, nOctaveLayers,
                 string("高斯金字塔"));
...
void myDisplay::write_keys_image(const Mat &img,
                                 const vector<KeyPoint> &keypoints,
                                 Mat &keypoints_image) {
  keypoints_image = img.clone();
  for (auto beg = keypoints.cbegin(); beg != keypoints.cend(); ++beg) {
    circle(keypoints_image, Point(beg->pt.x, beg->pt.y), 3,
           Scalar(0, 0, 255), 1);
    arrowedLine(keypoints_image, Point(beg->pt.x, beg->pt.y),
                Point(beg->pt.x + beg->size * sin(beg->angle),
                      beg->pt.y + beg->size * cos(beg->angle)),
                Scalar(0, 255, 0), 1);
  }
}