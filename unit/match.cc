  ...
  for (auto &kp : kps2->elements) {
    ...
    for (int i = 0; i < result.size() && dist == 1e18; i++) {
      ...
      for (auto &tmp : result_tmp) {
        LOG("match p'(%d,%d) - p(%d,%d)", result.at(i)->x, result.at(i)->y, tmp->x, tmp->y);
      }
      for (int j = 0; j < result_tmp.size() && dist == 1e18; j++) {
        if ((*result_tmp[j]) == (*kp)) {
          kp_tmp = result.at(i);
          for (int index_d = 0; index_d < kp->descriptor.size(); index_d++) {
            dist += (kp->descriptor.at(index_d) - result.at(i)->descriptor.at(index_d)) *
                    (kp->descriptor.at(index_d) - result.at(i)->descriptor.at(index_d));
          }
        }
      }
    }
...
void _min_des_points(const std::shared_ptr<KeyPoint> &kp,const std::vector<std::shared_ptr<KeyPoint>> &kps,std::vector<std::shared_ptr<KeyPoint>> &result){
  std::priority_queue<std::pair<double, std::shared_ptr<KeyPoint>>,
  std::vector<std::pair<double, std::shared_ptr<KeyPoint>>>, _cmp> min_des_points;
  double dist = -1;
