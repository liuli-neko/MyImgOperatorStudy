...
void Rgb2Gray(const IMG_Mat& img, IMG_Mat& dimg, const std::vector<double>& w) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            dimg.at<uint8_t>(i, j) = w[0] * pixel[0] + w[1] * pixel[1] + w[2] * pixel[2];
        }
    }
}
...
void HistogramEqualization(const IMG_Mat& input_img, IMG_Mat& output_img) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            output_img.at<uint8_t>(i, j) = static_cast<uint8_t>(round(sum_p[input_img.at<uint8_t>(i, j)] * GRAY_MAX));
        }
    }
}
...
void CreateGaussBlurFilter(IMG_Mat& filter, const double& sigma, int radim) {
    ...
    for (int i = 0; i <= radim / 2; i++) {
        filter.at<double>(i + radim / 2) = filter.at<double>(radim / 2 - i) = gaussFunction(sigma, i * i, 0.5);
    }
...
void CreateGaussBlurFilter(IMG_Mat& filter, const double& sigma, int radim_h, int radim_w) {
    ...
    for (int i = 0; i <= radim_h / 2; i++) {
        for (int j = 0; j <= radim_w / 2; j++) {
            filter.at<double>(i + radim_h / 2, j + radim_w / 2) = filter.at<double>(radim_h / 2 - i, radim_w / 2 - j) =
                filter.at<double>(radim_h / 2 - i, radim_w / 2 + j) =
                    filter.at<double>(radim_h / 2 + i, radim_w / 2 - j) = gaussFunction(sigma, i * i + j * j, 1);
        }
    }
...
void filter2d_(const IMG_Mat& input_img, IMG_Mat& output_img, const IMG_Mat& filter) {
...
void ImgFilter(const IMG_Mat& img, const IMG_Mat& filter, IMG_Mat& dimg, const bool& is_resverse) {
    int width   = img.cols;
    int height  = img.rows;
...
Eigen::Matrix<std::complex<double>, Eigen::Dynamic, Eigen::Dynamic> ConvertMat2Eigen(const IMG_Mat& img) {
    int height = img.rows;
    int width  = img.cols;
...
IMG_Mat ConvertEigen2Mat(const Eigen::Matrix<std::complex<double>, Eigen::Dynamic, Eigen::Dynamic>& img) {
    int height     = img.rows();
    int width      = img.cols();
...
void _fft_core(std::shared_ptr<std::complex<double>[]> src, int lim, const bool& is_fft) {
    int len = 0;
    while ((1 << len) < lim) {
        len++;
    }
    ...
    for (int m = 1; m <= lim; m <<= 1) {
        ...
        for (int i = 0; i < lim; i += m) {
            ...
            for (int j = 0; j < (m >> 1); j++, w = w * wn) {
                ASSERT(i + j + (m >> 1) < lim, "i+j+(m >> 1) = %d,lim:%d", i + j + (m >> 1), lim);
...
void _fft2D(const IMG_Mat& img, IMG_Mat& dft_img, const bool& is_fft) {
    ...
    std::shared_ptr<std::complex<double>[]> tmp(new std::complex<double>[lim_width]());
...
void IFFT2D(const IMG_Mat& img, IMG_Mat& dft_img) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            float value             = static_cast<float>(temp_img.at<std::complex<double>>(i, j).real());
...
void DFT(const IMG_Mat& img, IMG_Mat& dft_img) {
    ...
    // LOG(INFO,"temp_img size(fftshift): (%ld,%ld)", temp_img.rows,
    // temp_img.cols);
    ...
    _dft_core(eigen_img, dft_mat,
              [](double x, double u, double N) { return std::exp(std::complex<double>(0, -2 * PI * x * u / N)); });
    // 转换为IMG_Mat
...
IMG_Mat GrayErode(const IMG_Mat& src, const IMG_Mat& struct_element) {
    ...
    IMG_Mat dst       = IMG_Mat(height, width, src.type());
    for (int i = struct_height / 2; i < height - struct_height / 2; i++) {
        for (int j = struct_width / 2; j < width - struct_width / 2; j++) {
            int value = 1e9;
            for (int m = -struct_height / 2; m < struct_height / 2; m++) {
                for (int n = -struct_width / 2; n < struct_width / 2; n++) {
                    if (i + m > height || i + m < 0 || j + n > width || j + n < 0)
                        continue;
                    if (struct_element.at<uint8_t>(m + struct_height / 2, n + struct_width / 2) > 0) {
                        value = std::min(value, (int)src.at<uint8_t>(i + m, j + n));
                    }
                }
            }
            value                 = value > 255 ? 0 : value;
            dst.at<uint8_t>(i, j) = value;
        }
    }
    return dst;
}
        ...
        for (int j = struct_width / 2; j < width - struct_width / 2; j++) {
            ...
            for (int m = -struct_height / 2; m < struct_height / 2; m++) {
                for (int n = -struct_width / 2; n < struct_width / 2; n++) {
                    ...
                    if (struct_element.at<uint8_t>(m + struct_height / 2, n + struct_width / 2) > 0) {
                        value = std::max(value, (int)src.at<uint8_t>(i + m, j + n));
                    }
                }
            }
            dst.at<uint8_t>(i, j) = value;
...
IMG_Mat GrayOpening(const IMG_Mat& src, const IMG_Mat& struct_element) {
    return GrayDilate(GrayErode(src, struct_element), struct_element);
}
IMG_Mat GrayClosing(const IMG_Mat& src, const IMG_Mat& struct_element) {
    return GrayErode(GrayDilate(src, struct_element), struct_element);
}
} // namespace MY_IMG